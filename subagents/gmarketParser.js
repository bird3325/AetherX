// Gmarket SERP HTML Parser Sub-agent
window.GmarketParser = {
  // Gmarket 상품 리스트 아이템 탐색
  ...
  parseElement: function(el) {
    try {
      ...
      const linkEl = el.querySelector('a.link__item') || 
                     el.querySelector('a[href*="item.gmarket.co.kr"]') ||
                     el.querySelector('a[href*="gmarket.co.kr/Item"]') ||
                     el.querySelector('a');
      ...
      if (!productId) {
        const idMatch = url.match(/goodscode=(\d+)/) || url.match(/itemcode=(\d+)/) || url.match(/Item\?code=(\d+)/);
      ...
      console.error("Error parsing Gmarket element: ", e);
      return null;
    }
  },
